...
- **복제 효과 4종**(copy.ts): 빛이 된 나방=선택한 한 방향(copyDir)의 마지막 재능 1줄 / 들불=모든 인접 석판의 마지막 1줄씩 / 우주의 균열=좌우 석판의 중위+레중+절대자 전부 / 별들의 고향=인접 석판의 중위만
...
- 인펙션(낙인 재능 20% 투영) 계산 반영
- 우주의 균열: 한 변에 여러 석판이 닿을 때 전부 복제로 가정(게임 미확인), 복제 연쇄 여부 미확인
- fangzhu(추방) 대각선 형태 비연결 확인, 핵심 pts는 신 보드 것만 존재(직업 보드 null)
