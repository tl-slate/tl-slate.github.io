...
- `src/engine/` — geometry(회전·절대좌표), board(배치 유효성), tiers(등급 라벨·슬롯 수락), copy(복제 효과), judgment(심판 ×1.7), infection(인펙션 ×0.2 투영), aggregate(집계), rehydrate(저장분을 최신 정의로 재교정)
...
- 인펙션 미확인 2가지: ①투영받은 줄이 심판 ×1.7도 받는지(현재 받는 것으로 구현 — 복제와 동일 원칙. 단 명왕 석판은 캐릭터당 1개라 실전에선 동시 존재 불가) ②투영 수치가 반올림돼 0이 되는 재능(예: "공격 스킬 레벨 +1" × 0.2 = 0.2 → 0)의 게임 내 실제 처리
